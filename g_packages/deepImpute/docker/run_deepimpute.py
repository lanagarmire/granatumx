...
def main():
    ...
    ax[0].set_xlabel("Cells", **LABELS_PARAMS)
    ax[1].set_xlabel("Cells", **LABELS_PARAMS)
    ax[0].set_ylabel("Genes", **LABELS_PARAMS)
    ax[0].set_title("raw (log)", **LABELS_PARAMS)
    ax[1].set_title("imputed (log)", **LABELS_PARAMS)

    gn.add_current_figure_to_results("Heatmaps")

    nb_genes = np.sum([len(net.targetGenes) for net in model.networks])
    ...
    message = "\n".join(
        [
            "  - Number of imputed genes: **{0}**",
            "  - Percentage of dropout entries *before* imputation: **{1:.2f}%**",
            "  - Percentage of dropout entries *after* imputation: **{2:.2f}%**",
            "  - Accuracy (R2 score) on masked data: **{3:.2f}**",
        ]
    ).format(
        nb_genes,
        calc_dropout(data) * 100,
        calc_dropout(imputed) * 100,
        model.score(data),
    )

    gn.add_result(message, data_type="markdown")

    assay["matrix"] = imputed.T.tolist()
