...
def deepImpute(data, NN_lim='auto', n_cores=10, cell_subset=None, imputed_only=False, seed=0, **NN_params):
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='scRNA-seq data imputation using DeepImpute.')
    ...
    parser.add_argument('--cell-axis', type=str, choices=['rows','columns'], default=0,
                        help='Cell dimension in the matrix. Default: rows')
    parser.add_argument('--limit', type=str, default='auto',
    ...
    if args.cell_axis == 'columns':
        data = data.T
    imputed = deepImpute(data,n_cores=args.cores, NN_lim=args.limit, cell_subset=args.subset)
