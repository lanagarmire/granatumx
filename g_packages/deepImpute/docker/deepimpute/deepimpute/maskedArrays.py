...
class MaskedArray(object):
    ...
    def getMasked(self,rows=True):
        ...
        if rows:
            while compt < self.shape[0]:
                yield [self.data[compt,idx] for idx in range(self.shape[1])
                       if not self.binMask[compt,idx]]
                compt += 1
        else:
            while compt < self.shape[1]:
                yield [self.data[idx,compt] for idx in range(self.shape[0])
    ...
    def copy(self):
        args = {'data':self.data.copy(),
                'mask':self.binMask.copy()}
        return MaskedArray(**args)

    def get_probs(self,vec):
        return {'exp': expon.pdf(vec,0,20),
                'uniform': np.tile([1./len(vec)],len(vec))}.get(self.distr)
        ...
        for c in range(self.shape[0]):
            ...
            if cells_c_pos.size > 5:
                ...
                if n_masked >= cells_c_pos.size:
                    print('Warning: too many cells masked for gene {} ({}/{})'.
                          format(c,n_masked,cells_c_pos.size))
                    n_masked = 1+int(0.5*cells_c_pos.size)

                masked_idx = np.random.choice(cells_c_pos.size,n_masked,
                                              p=probs/probs.sum(),
                                              replace=False)
