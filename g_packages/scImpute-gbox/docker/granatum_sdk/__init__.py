...
class Granatum:
    ...
    def add_current_figure_to_results(self, description=None, zoom=2, width=750, height=650, dpi=100):
        ...
        self.results.append(
            {"type": "png", "width": width, "height": height, "description": description, "data": image_b64}
        )
    ...
    def data_frame_from_assay(self, assay):
        return pd.DataFrame(np.array(assay.get("matrix")).T, columns=assay.get("geneIds"), index=assay.get("sampleIds"))
    ...
    def pandas_from_assay(self, assay):
        return pd.DataFrame(np.array(assay.get("matrix")).T, index=assay.get("sampleIds"), columns=assay.get("geneIds"))

    def assay_from_pandas(self, df):
        return {
            "matrix": df.where((pd.notnull(df)), 0).values.T.tolist(),
            "sampleIds": df.index.values.tolist(),
            "geneIds": df.columns.values.tolist(),
        }
